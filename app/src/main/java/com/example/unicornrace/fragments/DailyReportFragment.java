...
import com.example.unicornrace.Services.MovementSpeedService;
...
public class DailyReportFragment extends Fragment implements ReportAdapter.OnItemClickListener, SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private final ServiceConnection mMovementSpeedServiceConnection = new ServiceConnection() {
    ...
    private void bindMovementSpeedService(){
        ...
        if(movementSpeedBinder == null && isVelocityEnabled){
            ...
            requireActivity().getApplicationContext().startService(serviceIntent);
            requireActivity().getApplicationContext().bindService(serviceIntent, mMovementSpeedServiceConnection, Context.BIND_AUTO_CREATE);
        }
    }

    private void unbindMovementSpeedService(){
        if(movementSpeedBinder != null && movementSpeedBinder.getService() != null){
            requireActivity().getApplicationContext().unbindService(mMovementSpeedServiceConnection);
            movementSpeedBinder = null;
        }
        Intent serviceIntent = new Intent(getContext(), MovementSpeedService.class);
    ...
    public void onTitleClicked() {
        ...
        DatePickerDialog dialog = new DatePickerDialog(requireContext(), R.style.AppTheme_DatePickerDialog, (view, year1, monthOfYear, dayOfMonth) -> {
            ...
            DailyReportFragment.this.day.set(Calendar.YEAR, year1);
    ...
    public void onWalkingModeClicked(int id) {
        if (menuWalkingModes.containsKey(id)) {
            ...
            AlertDialog.Builder alert = new AlertDialog.Builder(this.requireContext(), R.style.AppTheme_Dialog);
