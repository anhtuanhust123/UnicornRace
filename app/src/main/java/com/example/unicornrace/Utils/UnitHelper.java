...
public class UnitHelper {
    ...
    public static String formatKilometersPerHour(double kmh, Context context){
        return formatString(kilometersPerHourToUsersVelocityUnit(kmh, context), context) + usersVelocityDescription(context);
    }
    ...
    public static FormattedUnitPair formatKilometers(double km, Context context){
        ...
        if(kilometerInUsersLenghtUnit < 0.1){
            return new FormattedUnitPair(formatString(kilometerToUsersSmallLengthUnit(km, context), context), usersSmallLengthDescriptionShort(context));
        }else{
            return new FormattedUnitPair(formatString(kilometerInUsersLenghtUnit, context), usersLengthDescriptionShort(context));
        }
    }
    ...
    public static FormattedUnitPair formatCalories(double kcal, Context context){
        ...
        if(unit_key.equals("J")) {
            ...
            if (joule < 100) {
                return new FormattedUnitPair(formatString(joule, context), context.getString(R.string.joules));
            } else {
                return new FormattedUnitPair(formatString(joule / 1000, context), context.getString(R.string.kilojoules));
            }
        }else {
            if (kcal < 100) {
                return new FormattedUnitPair(formatString(kcal / 1000, context), context.getString(R.string.summary_card_calories));
            } else {
                return new FormattedUnitPair(formatString(kcal, context), context.getString(R.string.summary_card_kilocalories));
            }
        }
    }

    private static String formatString(double d, Context context){
        return String.format(context.getResources().getConfiguration().locale, "%.2f", d);
    }
