...
*/
package com.example.unicornrace.Adapter;
...
import com.example.unicornrace.Models.ActivityChartDataSet;
...
public class ReportAdapter extends RecyclerView.Adapter<ReportAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        switch (getItemViewType(position)) {
            case TYPE_CHART:
                ...
                barChartViewHolder.mChart.getXAxis().setValueFormatter((AxisValueFormatter) new ArrayListAxisValueFormatter(barChartXValues));
            ...
            case TYPE_DAY_CHART:
                ...
                for (Map.Entry<String, ActivityChartDataSet> dataEntry : dataMap.entrySet()) {
                    ...
                    if (dataEntry.getValue() != null) {
                        ...
                        Map.Entry prevChartEntry, chartEntry;
                        if (i == 0) {
                            prevChartEntry = (Map.Entry) new Entry(0, 0);
                        }else{
                            prevChartEntry = (Map.Entry) new Entry(i - 1, lastValue);
                        }
                        chartEntry = (Map.Entry) new Entry(i++, val);
                        ((LineDataSet) dataSets.get(dataSets.size() - 1)).getValues().add((Entry) prevChartEntry);
                        ((LineDataSet) dataSets.get(dataSets.size() - 1)).getValues().add((Entry) chartEntry);
                        // remember variables
                        lastValue = val;
    ...
    private LineDataSet getNewChartLineDataSet(Context context, String label) {
        LineDataSet chartLineDataSet = new LineDataSet(new ArrayList<>(), label);
        chartLineDataSet.setAxisDependency(YAxis.AxisDependency.LEFT);
    ...
    public static class ArrayListAxisValueFormatter implements AxisValueFormatter, ValueFormatter {
    ...
    public static class DoubleValueFormatter implements ValueFormatter {
